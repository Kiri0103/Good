...
- **実装済み**: 様式１（基本情報）, 様式２（発電設備概要：希望受電電圧・予備電線路系）, 様式３の４（逆変換装置＝PCS）, 様式４の１（変圧器・線路）。
- **一部のみ**: 様式２の希望時期（年月日）・電源種別・自家消費電力・定格出力合計・受電電力は入力セル要確認のため未収録。
- **未実装（順次拡張）**: 様式４の２（受電設備）, 様式４の３（給電情報）。
...
- [x] **AK1T 様式転記**: 様式１（基本情報）, 様式２（概要：受電電圧・予備線）, 様式３の４（PCS）, 様式４の１（変圧器・線路）
- [ ] **AK1T 様式転記（拡張）**: 様式２の希望時期/電源種別/出力, 様式４の２（受電設備）等
