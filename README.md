...
実プロジェクトの公開情報をもとに再構成した案件も同梱（出力・容量・立地・採用機器
などの公開値を反映し、非公開項目は一般構成で補い ★要確認 を明記）:

- `cases/msb_yokohama.yaml`: MIRARTH 横浜 特別高圧系統用蓄電池（12MW/65.8MWh）。
- `cases/sget_sapporo.yaml`: SGET（スパークス）／関西電力／JA三井リース 札幌
  系統用蓄電所（50MW/175.5MWh、SUNGROW PowerTitan 2.0 ×35、EPC 東芝、2028年運開）。
  ΔV の力率遅れケースが +3.12% で 3% 上限をわずかに超え、力率運用の確定が要る例。

これらは公開情報を参照した**設計検証用の再構成データ**であり、実申込には正式値での
置換が必要。
